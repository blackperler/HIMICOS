      ...
      IMPLICIT NONE
         ...
         allocate(ex_data(leng,2))                            !create an array for the data
      ...
      !initial guess of parameters
         DOUBLE PRECISION, PARAMETER:: D_ig = 8.0876d-14      !diffusivity, initial guess
         DOUBLE PRECISION, PARAMETER:: s2_ig = 2.2584d-29     !adhesion for phase2, initial guess
         DOUBLE PRECISION, PARAMETER:: s1_ig = 3.6791d-32     !adhesion for phase1, initial guess
         DOUBLE PRECISION, PARAMETER:: b2_ig = 1.2471d-05     !desorption for phase2, initial guess
         DOUBLE PRECISION, PARAMETER:: b1_ig = 0.0d0          !desorption for phase1, initial guess
         DOUBLE PRECISION, PARAMETER:: L_ig = 9.0810d-06      !radius of the particle, initial guess
         DOUBLE PRECISION, PARAMETER:: rho0_ig =  0.53656     !initial radius of the core, initial guess
!         DOUBLE PRECISION, PARAMETER:: gamm_ig =  1.!*Surf(L) !gamma parameter for the nuceation phase, initial guess
!         DOUBLE PRECISION, PARAMETER:: tgamm_ig =  29.        !duration of the nucleation phase, initial guess
         DOUBLE PRECISION, PARAMETER:: gamm_ig =  50.          !gamma parameter for the nuceation phase, initial guess
         DOUBLE PRECISION, PARAMETER:: tgamm_ig =  29.        !duration of the nucleation phase, initial guess
         DOUBLE PRECISION, PARAMETER:: m_ig =  0.98205        !linear transformation factor, initial guess
         DOUBLE PRECISION, PARAMETER:: shift_ig = -0.085475   !linear transforation shift, initial guess
      !parameters, span
...
!         bb1 = linear(minb1, maxb1,  params(5))              !;bb1=b1_ig      !desorption for phase1
         ...
         gamm=   linear(mingamm,maxgamm,params(9))*Surf(L)  !;gamm= 5.d-12*Surf(L)
         tgamm=  linear(mintgamm,maxtgamm,params(10))       ;tgamm= 0.
         ...
         min_rho = 0.01 * L                                                   !size of the core to be ignored
...
!         c1 = c1_div_c2*c2
         ...
         allocate(r0(xNOP), U0(xNOP))                                         !prepare arrays for concentration distribution
         r0 = rho0 + [(i, i=0,xNOP-1)]/dble(xNOP-1)*(L-rho0)                  !fill the grid nodes
 ...
 SUBROUTINE NextStep2layer(t,t1,U)              ! 2-layer proceeder: takes current time, previous time, and previos state; return current state
   USE Parabolic1D, ONLY: derx, LlinRnonlin, DIRICHLET_I, NEUMANN_II !use some stuff for solving parabolic 1D PDE
   ...
   IMPLICIT NONE
     ...
     if(t1 < tgamm) then                                             !the nucleation stage
        ...
        if(rho>min_rho) then                                         !if the core is not too small
           drho = eta * D*derx(V,1,L,rho)/(c1-U(left))               !evaluate its derivative
           rho  = rho + drho*dt                                      !the Euler time step for the core radius
           rho  = max(rho,0.)                                        !repair negative value
           call LlinRnonlin(dt, V, A,B,B1,E,F, DIRICHLET_I, LBC, RBC, L, rho, 0.0d0, drho) !call the solver for a PDE with linear left BC and nonlinear right BC
        else                                                         !if there is no more core 
           drho = 0.0d0                                              !no further change
           rho  = 0.0d0                                              !no core means no core
           call LlinRnonlin(dt, V, A,B,B1,E,F, NEUMANN_II, LBC2, RBC, L, rho, 0.0d0, drho) !call the same solver but with different left BC
        end if
   ...
   CONTAINS                                                          !PDE coefficients to be given to PDE solvers
      ...
      PURE FUNCTION E(i)                                             !Eu
       IMPLICIT NONE
         DOUBLE PRECISION:: E
         INTEGER, INTENT(IN):: i                                     !index of the grid point    
         E = 0.0d0                                                   !nothing in our case
      END FUNCTION

      PURE FUNCTION F(i)                                             !free term
       IMPLICIT NONE
         DOUBLE PRECISION:: F
         INTEGER, INTENT(IN):: i                                     !index of the grid point    
         F = 0.0d0                                                   !nothing in our case
      END FUNCTION
 ...
 END SUBROUTINE
      ...
      DOUBLE PRECISION:: disb                                        !disbalance: violation of the conservation
         ...
         disb = (was-is-gone)/(Vol(L)*c2)*100.                       !evaluate disbalance
         ...
         write(42, *) was,is,gone,Vol(L)*c2                          !report the amounts of was, is now, and gone/arrived
         close(42)                                                   !close files
         CONTINUE
      END SUBROUTINE
