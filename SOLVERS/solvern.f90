 ...
 allocate(Y(0:nlayers-1, n_unknowns))                                      !allocate the phase vector: all variables at any time will be here
 ...
 Q(0,2) = Quantity(tv(0),Y(0,:))                                        !get the initial value of the quantity to be compared (real IC to be added inside)

 do n = 1,NOP                                                            !loop over the time grid
   tv(n) = tv(n-1) + dt                                                  !the time step
   if(n >= nlayers)  Y(0:nlayers-2,:) = Y(1:nlayers-1,:)
   minn = max(0,n-nlayers+1)
   maxn = min(n,nlayers-1)
   call NextStepnlayer(tv(minn:n),Y(0:maxn,:))  ! User provided n-layer proceeder: takes up to n times, up to n states, returns the current state in Y(max,:)
   if(EarlyQuit(tv(minn:n),Y(0:maxn,:))) then               ! check for early stop
          Q(n:NOP,1) = Q(n-1,1);                ! if stop, then quantity is unchanged further on
          Q(n:NOP,2) = Q(n-1,2);
          exit                                  !stop the loop
   end if
   Q(n,1) = IndependentVar(tv(minn:n))               !evaluate the data pair
   Q(n,2) = Quantity(tv(minn:n),Y(0:maxn,:))
