 ...
 allocate(Y(0:NOP, n_unknowns))                                          !allocate the phase vector: all variables at any time will be here
 ...
 do n = 1,NOP                                                            !loop over the time grid
   ...
   call NextStepAlllayer(tv(0:n),Y(0:n,:))  ! User provided multilayer proceeder: takes all times up to now, all states, returns current state in intent(out) Y(n,:)
   if(EarlyQuit(tv(0:n),Y(0:n,:))) then     ! check for early stop
          Q(n:NOP,1) = Q(n-1,1);            ! if stop, then quantity is unchanged further on
          Q(n:NOP,2) = Q(n-1,2);
          exit                              !stop the loop
   end if
   Q(n,1) = IndependentVar(tv(0:n))         !evaluate the data pair
   Q(n,2) = Quantity(tv(0:n),Y(0:n,:))
